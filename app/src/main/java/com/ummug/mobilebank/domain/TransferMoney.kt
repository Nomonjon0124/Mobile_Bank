...
class TransferMoney @Inject constructor(
    ...
    suspend operator fun invoke(from_card_id:Int,money:Int?,pan:String?):State{

        if (money==null || money<5000)return State.Error(ErrorCodes.MONEY)
        if (pan==null || pan.length!=16)return State.Error(ErrorCodes.PEN_ERROR)

        try {
           val response= transferRepository.transferMoney(TransferEntity(money,from_card_id,pan),"Bearer ${settings.usetoken}")
